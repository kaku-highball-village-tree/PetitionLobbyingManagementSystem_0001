...
PROMPT_IMAGE_TO_TEXT = (
    ...
    "自然な文章に直さないでください。\n"
    "推測で補完しないでください。\n"
    "画像に見えている文字を、できるだけ原文どおりに、そのまま出力してください。\n"
    "読めない文字は [判読不能] と書いてください。\n"
    "レイアウト上、改行がある場合は、改行も可能な限り維持してください。\n"
    "相談メモ、陳情、請願、役所文書、道路修繕相談などの、日本語長文OCR用途です。\n"
    "自然さより、原文忠実性を優先してください。\n"
    "以下は禁止です: 自然な文章への書き換え、意味補完、読みやすい文章への修正、OCR結果の意訳、勝手な漢字変換、勝手な語句修正。\n"
    "出力は画像内文字列の忠実抽出を最優先し、OCRログに近い思想で、grep可能性を重視してください。\n"
    "ファイル名案は不要です。\n"
    "タイトル案は不要です。\n"
    "出力は本文テキストだけにしてください。\n"
    "\n"
